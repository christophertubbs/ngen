...
class HY_HydroLocation : public GM_Object
{
    public:
        ...
        HY_HydroLocation(polygon_t input_shape, HY_HydroLocationType input_type, point_t input_point) :
            _shape(input_shape), _type(input_type), _point(input_point), _realized_nexus()
        {}

        //! Constructor for when attached nexus is know
        HY_HydroLocation(polygon_t input_shape, HY_HydroLocationType input_type, point_t input_point, std::shared_ptr<HY_HydroNexus> nexus) :
            _shape(input_shape), _type(input_type), _point(input_point), _realized_nexus(nexus)
        {}

        //! get the shape of the location
        polygon_t geometry() { return _shape; }
        const polygon_t geometry() const { return _shape; }
        ...
        void set_nexus(std::shared_ptr<HY_HydroNexus> nexus) {
            this->_realized_nexus = nexus;
        }

    private:

        polygon_t _shape;                                   //!< shape for location
        point_t _point;
