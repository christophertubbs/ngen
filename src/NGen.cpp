...
int main(int argc, char *argv[]) {
    ...
    realization::Realization_Manager manager = realization::Realization_Manager(REALIZATION_CONFIG_PATH);
    manager.read();
    ...
    for(int time_step = 0; time_step < 720; time_step++)
    {
      ...
      for (std::pair<std::string, std::shared_ptr<realization::Realization>> realization_pair : manager ) {
        double response = realization_pair.second->get_response(0, time_step, 3600.0, &pdm_et_data);
        std::cout<<"\tCatchment "<<realization_pair.first<<" contributing "<<response<<" m/s to "<<catchment_to_nexus[realization_pair.first]<<std::endl;
        catchment_outfiles[realization_pair.first] << time_step <<", "<<response<<std::endl;
        response = response * boost::geometry::area(nexus_collection->get_feature(realization_pair.first)->geometry<geojson::multipolygon_t>());
        std::cout << "\t\tThe modified response is: " << response << std::endl;
        nexus_realizations[ catchment_to_nexus[realization_pair.first] ]->add_upstream_flow(response, catchment_id[realization_pair.first], time_step);
      }
