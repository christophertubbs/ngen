...
Alternate_Tshirt_Realization::Alternate_Tshirt_Realization(
    ...
    : HY_CatchmentArea(forcing_config), catchment_id(catchment_id), params(params), dt(t)
{
    ...
    state[0] = std::make_shared<tshirt::tshirt_state>(tshirt::tshirt_state(soil_storage_meters*0.5, groundwater_storage_meters*0.75, nash_storage));
...
Alternate_Tshirt_Realization::Alternate_Tshirt_Realization(
        ...
        time_step_t t
) : Alternate_Tshirt_Realization::Alternate_Tshirt_Realization(forcing_config, soil_storage_meters, groundwater_storage_meters,
...
void Alternate_Tshirt_Realization::add_time(time_t t, double n) {
...
double Alternate_Tshirt_Realization::get_response(double input_flux, Alternate_Tshirt_Realization::time_step_t t, time_step_t dt, void* et_params)
{
  return get_response(input_flux, dt, std::make_shared<pdm03_struct>( *(pdm03_struct*) et_params ));
}

double Alternate_Tshirt_Realization::get_response(
  double input_flux,
  time_step_t t,
  const shared_ptr<pdm03_struct> &et_params
    ...
    // Divide the input flux by the forcings AORC DSWRF surface W per meters squared and add it to the precip
    int error = model->run(t, ((input_flux / this->forcing.get_AORC_DSWRF_surface_W_per_meters_squared()) + precip) * dt/1000, et_params);
